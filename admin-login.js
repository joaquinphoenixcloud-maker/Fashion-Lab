// admin-login.js
// SUPABASE_URL, supabase client, and AUTH_DOMAIN are assumed to be loaded from scriptOOO.js
...
// ----------------------------------------------------
// Step 1: Login with Email/Password, check Admin Role, and send OTP
// ----------------------------------------------------
async function sendOtp() {
    ...
    // 1. Email/Password ဖြင့် Sign In ဝင်ပါ
    ...
    const { data: profile, error: profileError } = await supabase
        .from('profiles') // **သင်၏ Profile Table Name ကို ပြောင်းရန် (အများအားဖြင့် 'profiles' ဖြစ်သည်)**
        .select('role, phone')
        .eq('id', user.id)
        .single();
    
    if (profileError || !profile || profile.role !== 'admin') {
        showMessage('Access Denied: This account is not an Admin account.', true);
    ...
    // AUTH_DOMAIN ကို scriptOOO.js မှ ယူသုံးပါသည်
    ...
    if (otpError) {
        showMessage('Failed to send OTP. Check your Supabase configuration and logs.', true);
        await supabase.auth.signOut();
        return;
    }
    
    // 5. အောင်မြင်ပါက OTP Section ကို ပြပါ
    showMessage('OTP code sent successfully to your phone! Please verify below.', false);
...
async function verifyOtp() {
    ...
    showMessage('Verifying code...', false);
